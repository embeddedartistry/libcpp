	...
	struct vtable
	{
		...
		explicit constexpr vtable() noexcept
			: invoke_ptr{[](storage_ptr_t, Args&&...) -> R {
				  // TODO: throw std::bad_function_call();
			  }},
		...
		template<typename C>
		explicit constexpr vtable(wrapper<C> unused __attribute__((unused))) noexcept
			: invoke_ptr{
				  [](storage_ptr_t storage_ptr,
					 Args && ... args) noexcept(noexcept(std::declval<C>()(args...)))
					  ->R{return (*static_cast<C*>(storage_ptr))(std::forward<Args>(args)...);
	}
} // namespace inplace_function_detail
, copy_ptr{[](storage_ptr_t dst_ptr,
			  storage_ptr_t src_ptr) noexcept(std::is_nothrow_copy_constructible<C>::value)
			   ->void{::new(dst_ptr) C{(*static_cast<C*>(src_ptr))};
} // namespace stdext
}
, relocate_ptr{[](storage_ptr_t dst_ptr,
				  storage_ptr_t src_ptr) noexcept(std::is_nothrow_move_constructible<C>::value)
				   ->void{::new(dst_ptr) C{std::move(*static_cast<C*>(src_ptr))};
static_cast<C*>(src_ptr)->~C();
}
}
, destructor_ptr{[](storage_ptr_t src_ptr) noexcept->void{static_cast<C*>(src_ptr)->~C();
}
}
{
}
...
template<typename Signature,
		 ...
		 size_t Alignment =
			 std::alignment_of<inplace_function_detail::aligned_storage_t<Capacity>>::value>
...
class inplace_function<R(Args...), Capacity, Alignment>
{
  public:
	...
	template<typename T, typename C = std::decay_t<T>,
			 typename = std::enable_if_t<!(std::is_same<C, inplace_function>::value ||
	...
	inplace_function& operator=(inplace_function&& other) noexcept
	{
		if(this != std::addressof(other))
		{
			...
			vtable_ptr_ =
				std::exchange(other.vtable_ptr_,
	...
	{
		static_assert(
			inplace_function_detail::is_valid_inplace_dst<Cap, Align, Capacity, Alignment>::value,
